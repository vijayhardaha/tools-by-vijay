...
import { EXAMPLES } from '@/components/tools/replace-quotes/examples';

describe('ReplaceQuotes tool', () => {
  it('renders example buttons and the info/FAQ sections', () => {
    render(<ReplaceQuotes />);
    expect(screen.getByRole('button', { name: EXAMPLES[0].label })).toBeInTheDocument();
    expect(screen.getByText('Frequently Asked Questions')).toBeInTheDocument();
  });

  it('loads an example and transforms the quotes', async () => {
